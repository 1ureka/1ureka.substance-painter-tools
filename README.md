# 1ureka 的 Substance Painter 工具集
...
| **濾鏡效果圖層**           | 特定補償參數         | 濾鏡效果需要根據縮放比例對特定參數進行物理補償，以維持相同的視覺效果。                                  | Blur (模糊)、Warp (扭曲)、Blur Slope (坡度模糊) 等                                    |
...
- **不支援的濾鏡效果**：只有特定濾鏡（Blur、Warp、Blur Slope）會進行參數補償，其他濾鏡會被跳過。
...
A: **濾鏡效果會根據縮放比例自動進行物理參數補償**，以確保視覺效果一致。

例如，當紋理放大 2 倍時，代表一個紋理被 tile 2 次：

- **Blur (模糊)** 的模糊強度會自動減半，確保模糊半徑在新的物理空間中保持不變
- **Warp (扭曲)** 的扭曲強度會減半，同時噪點縮放會加倍，維持扭曲效果的正確空間關係
- **Blur Slope (坡度模糊)** 會進行與 Warp 類似的補償

目前僅支援部分常用濾鏡的自動補償。如果濾鏡不在支援列表中，該圖層會被跳過並在結果報告中說明。

> [!NOTE]
>
> **關於濾鏡的旋轉參數：**
> 濾鏡效果通常沒有旋轉的概念，因此旋轉參數對濾鏡圖層不生效。本擴展只會調整濾鏡的縮放相關參數。
